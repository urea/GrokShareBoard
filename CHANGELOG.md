...
### 修正
- **Grok V2作品のプロンプト取得を新しいAsset Repository仕様へ追従**:
  - 従来のMedia Post APIが空のプロンプトを返すV2作品では、公開Asset APIから生成種別ごとのプロンプトを取得するフォールバックを追加しました。
  - `inputAssets`を元画像・上位作品として追跡し、従来どおり`post_prompt_sources`へ分離保存します。
  - 従来形式でプロンプトを取得できる作品は既存経路を維持し、新経路は必要な場合だけ使用します。
  - 移行前に`no_prompt`となった作品を再取得する運用オプションと、特定のGrok UUIDだけを再取得するオプションを追加しました。

## [1.13.1] - 2026-07-20

### 修正
- **Grok Imagine新形式（isV2）の投稿・動画取得に対応**:
  - Grok投稿APIが返す実際の`mediaUrl`と`thumbnailImageUrl`を既存Vercel中継で安全に解決し、`assets.grok.com`へ移行した新形式でもプレビュー・投稿できるようにしました。
  - 従来の予測URLが404になることを「未公開」と誤判定していた新規投稿ブロックを解消しました。
  - 新形式の動画URLを一覧カード・詳細画面・動画結合ツールで利用できるようにしました。
  - 旧形式の`imagine-public.x.ai`投稿と既存データ向けのフォールバックは維持しています。

## [1.13.0] - 2026-07-13

### 追加
- **ブラウザ内で完結するGrok動画結合ツールを追加**:
  - `/tools/video-join` で、Grokから保存した2〜10本のMP4を入力順に結合できるようにしました。
  - FFmpeg WebAssemblyを実行時だけ遅延読み込みし、動画・URL・完成ファイルをサーバーやSupabaseへ送らず利用者PC内だけで処理します。
  - まず再エンコードなしの高速結合を試し、形式差がある場合のみH.264/AACの互換MP4へ変換します。
  - Grok URLのローカルチェックリスト、ファイルの並べ替え、容量・尺表示、進捗、日時付きファイル名でのダウンロードを追加しました。
  - GrokShareBoard本体と同じUUIDベースの公開MP4 URL構築を使い、各Grok URLに「動画を保存」ボタンを追加しました。
  - 開発時のFast Refreshやページ再読み込みが発生しても、入力済みGrok URLを同じタブ内で復元するようにしました。
  - メイン画面のヘッダーとフッターから動画結合ツールへ移動できるようにしました。

## [1.12.3] - 2026-07-12

### 修正
- **カード内表示の統一性を修正**:
  - 意味のない強調になっていた「詳細」ボタンの青色を廃止し、3つの操作ボタンで通常時・ホバー時とも同一デザインを共有するようにしました。
  - コメント・閲覧・Grok遷移の指標を、ラベルと数値が横並びになる1行表示へ戻しました。
  - 操作段と指標段の高さを同じ28pxに揃え、カード下部のまとまりを改善しました。

## [1.12.2] - 2026-07-12

### 変更
- **カード内の操作・指標表示を整理**:
  - 「詳細」「Grokで見る」「Xで共有」を同じ幅の操作段へまとめ、表記を日本語中心に統一しました。
  - コメント数・閲覧数・Grok遷移数を別段に分け、数値と意味が一目で対応する表示へ変更しました。
  - スマートフォンの2列表示でも文字切れや横スクロールが発生しないよう、固定寸法とレスポンシブ文字サイズを設定しました。

## [1.12.1] - 2026-07-09

### 変更
- **詳細モーダルの表示幅を拡張**:
  - プロンプト情報が増えたため、詳細モーダルを画面幅いっぱいに近い `96vw` へ広げました。
  - デスクトップでは右側の情報ペインを最大640pxまで確保し、動画/画像表示とプロンプト欄の両方を読みやすくしました。
  - モバイルでは従来どおり上下積みを維持し、横スクロールが出ないよう調整しました。

## [1.12.0] - 2026-07-09

### 追加
- **元画像・上位プロンプトの取得と表示に対応**:
  - `posts.prompt` は共有投稿そのもののプロンプトとして維持し、元画像など上位投稿のプロンプトは `post_prompt_sources` に分離して保存する構造を追加しました。
  - 詳細モーダルで「動画/画像プロンプト」「元画像プロンプト」「上位プロンプト」を分けて表示し、それぞれ個別にコピーできるようにしました。
  - 検索ビュー `posts_search_index` を追加し、説明・現行プロンプトに加えて元画像/上位プロンプトも検索対象にしました。
  - Grokプロンプト取得スクリプトで `originalPostId` の親子関係を追跡し、既定で3階層まで取得するようにしました。
  - GitHub Actions の `checkout` / `setup-node` を最新版へ更新し、取得ジョブを Node 24 と上位プロンプト取得に対応させました。

## [1.11.5] - 2026-07-07

### 変更
- **新規投稿時のGrok公開化確認を必須化**:
  - Grok側で「シェア」または「Xに投稿」による公開化が確認できるまで、新規投稿のPostボタンを押せないようにしました。
  - プレビュー失敗時の案内に「ダウンロード」「再生成」「延長」では公開化されないことを明記しました。
  - 既存投稿の編集・削除はこれまで通り可能です。
  - スマホ幅でヘッダー右側の補助リンクが横スクロールを発生させる問題を抑制しました。

## [1.11.4] - 2026-07-06

### 修正
- **再取得依頼ボタンの補足説明を追加**:
  - Grok側で公開化済みの場合のみ再取得対象に戻す操作であることを、ボタンのtitleで補足しました。

## [1.11.3] - 2026-07-06

### 変更
- **Grok公開化後の再取得依頼UIを追加**:
  - `access_denied` の投稿を定期取得で毎回再チェックしないよう、通常の自動取得対象から外しました。
  - 詳細画面に「公開化済み・再取得を依頼」ボタンを追加し、ユーザーがGrok側で公開化操作をした後だけ `pending` に戻せるようにしました。
  - 手動実行用の `--retry-access-denied` は残し、必要時のみ一括再チェックできるようにしています。

## [1.11.2] - 2026-07-06

### 修正
- **定期取得ワークフローの既定値を安定化**:
  - スケジュール実行時はスクリプト側の既定値で `access_denied` を再チェックし、手動実行で明示的にfalseを選んだ場合のみ除外するようにしました。

## [1.11.1] - 2026-07-06

### 変更
- **Grok公開化操作待ちの案内を明確化**:
  - プレビュー未生成時に、Grok側で「シェア」または「Xに投稿」を押して公開化する必要があることを案内するようにしました。
  - 投稿自体は止めず、公開化前はサムネイル・動画・元プロンプト取得が失敗する可能性があることを表示します。
  - `access_denied` は「公開化待ち」の可能性があるため、定期取得の再チェック対象に戻しました。

## [1.11.0] - 2026-07-06

### 追加
- **Grok元プロンプトのGitHub Actions定期取得**:
  - `pending` / `failed` の投稿を対象に、Grok側APIから元プロンプトを取得して `posts.prompt` に反映するActionsワークフローを追加しました。
  - 通常実行では `no_prompt`、`source_missing`、`access_denied` を終端ステータスとして再チェック対象から外します。
  - Actionsログにプロンプト本文を残さないよう、本文長とハッシュのみを出力する取得スクリプトを追加しました。

## [1.10.2] - 2026-07-06

### 変更
- **GitHub PagesデプロイActionの更新**:
  - GitHub ActionsのNode 24実行環境に合わせ、Pagesデプロイ関連Actionをv5へ更新しました。

## [1.10.1] - 2026-07-06

### 変更
- **Grok元プロンプト取得ステータス表示の細分化**:
  - プロンプト無し生成、Grok側で削除済み/存在しない投稿、アクセス制限による取得不可を、取得待ちと区別して表示するようにしました。
  - 終端扱いできる取得結果を再チェック対象から外せるよう、DBステータス分類に対応しました。

## [1.10.0] - 2026-07-06

### 変更
- **Grok元プロンプトと説明欄の分離**:
  - `posts.prompt` をGrok側から取得した元プロンプト専用欄として扱い、投稿者が入力する自由記述は新設の `posts.description` に分離しました。
  - 詳細モーダルでは「Grok元プロンプト」「説明・メモ」「コメント」を別セクションとして表示します。
  - Grok元プロンプトが未取得の場合は、取得待ちであり時間がかかる場合があることを表示します。
- **投稿フォームの説明欄対応**:
  - 投稿フォームの自由入力欄を「説明・メモ」に変更し、新規投稿では `prompt` を空、`description` を入力内容として保存するようにしました。
  - 投稿後にGrok元プロンプトを順次取得する旨の注意文を追加しました。
- **DB移行準備**:
  - 既存の `posts.prompt` 内容を `posts.description` へ移送するDB拡張を実施しました。
  - 旧promptの削除は、公開UIが新仕様へ反映された後に実行する後処理として分離しました。

## [1.9.6] - 2026-05-17

### 変更
- **Xカード種別をsummaryへ変更**:
  - 投稿内容の画像・動画サムネをX上に展開せず、サイト共通ロゴを小さなサムネイルとして表示する方針に合わせ、Twitter/Xカード種別を `summary_large_image` から `summary` に変更しました。
  - `og-image.png` を正方形のサイト共通ロゴ画像として追加し、既存のカード画像参照先が404にならないようにしました。

## [1.9.5] - 2026-05-16

### 変更
- **広告枠の自作Amazonバナーへの移行**:
  - 忍者AdMax（クリック型ディスプレイ広告）を撤去し、自作のAmazonアソシエイトバナー画像に置き換えました。
  - PC/SP出し分けを廃止し、320x100バナー1枚での統一表示に簡素化しました。
  - 外部広告スクリプトへの依存を排除したことで、広告が真っ白になる不具合が根本的に解消されます。
- **忍者AdMaxのアクセス計測用残存**:
  - 忍者AdMaxはインプレッション数によるアクセス数把握のため、フッター直前に1x1pxの不可視iframeとして残しています（ユーザーには見えません）。

## [1.9.3] - 2026-05-16

### 機能改善・リファクタリング
- **旧ビデオモーダルの廃止と詳細モーダルへの統合**:
  - サムネイルクリック時に開いていた「シンプルな画像/動画ビューア」を廃止し、代わりにv1.8.0でリッチ化した詳細モーダルが直接開くようになりました。
  - これにより、サムネクリック1回でメディア＋プロンプト＋コメント＋Shareボタンの全てにアクセスできるようになりました。
- **ビューカウント（Views）の全経路対応**:
  - 「詳細」ボタンクリック、URLパラメータ経由のアクセス、左右ナビゲーションなど、すべての経路でビューカウントが正しく加算されるようになりました。

## [1.9.2] - 2026-05-16

### バグ修正
- **シェアURLのbasePath欠落修正**:
  - Xシェアボタンで生成されるURLに、GitHub Pagesのサブパス（`/GrokShareBoard`）が含まれず404エラーとなる不具合を修正しました。

## [1.9.1] - 2026-05-16

### 機能追加・UI改善
- **一覧カードへのXシェアボタン追加**:
  - 各投稿カードの「詳細」「Grok」ボタンの横に「Share」ボタンを新設し、詳細画面を開かずとも一覧から直接Xへシェアできるようになりました。
- **ボタンデザインの統一**:
  - 「詳細」「Grok」「Share」の3つのボタンのテキスト色・borderスタイルを統一し、視覚的な一貫性を改善しました。

## [1.9.0] - 2026-05-16

### 新機能
- **URLパラメータによる詳細モーダル自動展開**:
  - `?postId=XXX` 付きのURLでサイトにアクセスすると、該当投稿の詳細画面（リッチモーダル）が自動的に開くようになりました。一覧に表示されていない投稿でも、Supabaseから直接取得して確実に表示します。
- **X（Twitter）シェアボタンの設置**:
  - 詳細モーダル内に「Share」ボタンを新設しました。クリックするとXの投稿画面が開き、テキストとURL（`?postId=XXX`形式）のみが渡されます。
  - 凍結リスク回避のため、OGP（プレビュー画像）は意図的に設定していません。Xのタイムライン上ではテキストリンクのみが安全に表示されます。

## [1.8.0] - 2026-05-16

### 新機能・UI改善
- **詳細モーダルのリッチ化（統合表示）**:
  - これまで「プロンプト・コメント用」と「画像/動画の拡大表示用」で分かれていたモーダルを、1つのリッチな詳細画面に統合しました。
  - 今後は「詳細」ボタンをクリックするだけで、メディア（画像や動画）とコメント・プロンプトを同じ画面内で同時に閲覧できるようになります。将来的なURL共有機能（パーマリンク）に向けた土台となるUI改修です。

## [1.7.4] - 2026-03-23

### UI/UX 改善
- **管理者モード起動トリガーの変更とリロードボタンの設置**:
  - タイトルロゴ（GrokShareBoard）のクリックを「ページ全体のリロード（F5と同等）」機能に変更し、一般ユーザーの利便性を向上させました。
  - 誤操作を防ぐため、管理者モード（隱し機能）の起動コマンドを「タイトルロゴ部分の連打」から「右横のバージョン番号（v1.x.x）の5回連打」に限定して移設しました。

## [1.7.3] - 2026-03-23

### 追加・改善機能
- **管理者モード: 投稿の強制削除機能**:
  - 管理者機能（ヘッダー連打による隠しモード）中に表示されるNSFWトグルボタンの横に、「強制削除（ゴミ箱）」ボタンを新設しました。
  - 誤操作防止のブラウザ確認ダイアログ（Confirm）を経由した後、SupabaseのDBからレコードを直接削除し、画面をリロードすることなくリアルタイムでリストからカードを消去するUXを実現しています。

## [1.7.2] - 2026-03-23

### UI/UX 改善
- **スポンサー広告枠のレスポンシブ・スマート化**:
  - 先ほど導入した忍者AdMaxの「300x250」枠が想定より大きく、テキスト横のレイアウトを重くしていたフィードバックを受け、デバイスごとの完全な出し分け（レスポンシブ）制御を実装しました。
  - **PC向けレイアウト**: よりスリムでスマートな「728x90（横長）」をテキストの下部（幅が広い場合は右側）へ配置し、バナーによる縦への無駄な膨張を解消しました。
  - **スマホ向けレイアウト**: 画面幅に合った「320x100」専用のバナーが表示されるようになり、あらゆるデバイスで美しいレイアウトと収益の最大化を両立しています。
  - 合わせて、説明文を特定のアフィリエイト（商品紹介）と受け取られかねない古い文章から「広告へのご関心」という適切なものに微修正しました。

## [1.7.1] - 2026-03-23

### 変更・機能追加
- **広告枠の差し替え (忍者AdMaxの導入)**:
  - 過去に撤去した「もしもアフィリエイト」等の代替として、アダルト（NSFW）コンテンツに正式対応している「忍者AdMax」の広告枠（300x250 レクタングル）を新設しました。
  - Next.js側のSPAハイドレーションとの競合を防ぐため、広告スクリプトを静的HTML (`ninja-admax.html`) に隔離し `iframe` を使って安全にコンポーネントへ読み込ませるアーキテクチャを引き継いでいます。

## [1.7.0] - 2026-03-17

### 新機能・UI改善
- **Grok新仕様（assets.grok.com移行）に伴う画像救済ロジックの導入**:
  - Grok側の仕様変更により、従来の予測URL（imagine-public.x.ai）では新着投稿の画像・動画が表示されなくなっていた問題を緊急対応しました。
  - **画像プロキシURL (?v=3) 方式の採用**: 従来のURLが404となった場合の最終フォールバックとして、Grok公式のメタデータ用プロキシURLを組み込みました。これにより、新仕様に移行した投稿であっても、画像プレビューだけは確実に表示・共有可能になりました。
  - ※動画（.mp4）については、新仕様ではユーザー固有IDの解析が必要なため、現在はGrokの元ページへ遷移して視聴していただく動線を推奨しています。

## [1.6.6] - 2026-03-16

### 新機能・UI改善
- **運営メッセージへのSNS宣伝協力のお願いを追加**:
  - 「運営よりご協力のお願い」セクションに、当サイトをX(Twitter)やNote等で宣伝・拡散していただくためのお願いを追記しました。
  - サーバー維持費の確保と認知度向上のため、ユーザーの皆様による自発的なご紹介を呼びかけています。

## [1.6.5] - 2026-03-06

### その他
- **広告掲載に関する免責事項の追記**:
  - Amazonアソシエイト・プログラムの審査および景品表示法（ステマ規制）への対応として、フッターに「Amazonのアソシエイトとして適格販売により収入を得ていること」および「アフィリエイト広告を掲載していること」を明記しました。

## [1.6.4] - 2026-03-05

### バグ修正
- **GitHub Pages 環境下での 404 エラー解消**:
  - 静的 HTML ファイル（rakuten-widget.html）を iframe で読み込む際、ベースパス（/GrokShareBoard/）の不一致により 404 となっていた問題を相対パス指定に切り替えることで解決しました。

## [1.6.1] - 2026-03-05

### バグ修正
- **楽天モーションウィジェットの表示不具合解消**:
  - 静止画バナーからリッチなウィジェット形式に変更した際、一部環境で表示が真っ白になる問題を、iframe（サンドボックス方式）に切り替えることで完全に解消しました。
  - レガシーな広告スクリプトがSPA（Next.js）の描画タイミングと競合する問題を根本から回避しています。

## [1.6.0] - 2026-03-05

### バグ修正
- **画像表示エラー（Main video playback failed）の解消**:
  - 静止画のみの投稿を表示する際に、誤ってビデオタグが読み込まれ、エラー画面が表示されてしまう問題を修正しました。
  - 動画ファイル（.mp4）の有無を厳密に判定するようにロジックを改善し、安定性を向上させました。

## [1.5.9] - 2026-03-05

### 新機能・UI改善
- **ブラウザの「戻る」ボタンへの対応**:
  - 詳細モーダルや動画プレビューを開いた際、URLハッシュ（#video, #prompt）を用いたHistory APIの制御を導入しました。
  - これにより、スマホの戻る操作やブラウザの戻るボタンで「モーダルのみを閉じる」自然な操作感が可能になり、意図しないサイト離脱を防止しました。

## [1.5.8] - 2026-03-05

### 新機能・UI改善
- **もしもアフィリエイト（楽天市場）の導入と運営メッセージの統合**:
  - サーバー維持費補填のため、もしもアフィリエイト経由での「楽天市場」バナーを導入しました。
  - 従来の「note誘導メッセージ」と「広告バナー」を1つの『運営サポートセクション』として統合。高さを圧縮し、メインコンテンツ（画像一覧）へのアクセスを妨げない非常にスリムなレイアウトに刷新しました。
  - 広告表示に対する運営側の背景（止むを得ずの決断）を明記し、ユーザーへ「バナー経由の買い物」または「noteでのチップ」による協力を仰ぐ誠実なメッセージ構成に変更しました。
  - 今後のAmazon提携審査を見据え、必要となる免責事項の準備（メモ）を完了。

## [1.5.7] - 2026-03-04

### 新機能・UI改善
- **note連携（支援・不具合報告窓口）の導入**:
  - サイト運営維持費（サーバー代）の確保とユーザーコミュニケーションのため、公式noteへの誘導リンクを実装しました。
  - 投稿ガイドと画像一覧の間に、コンパクトな案内バナーを設置。「不具合報告・要望はコメント欄」「支援はチップ機能」という役割分担を明記し、スムーズな外部連携を可能にしました。
  - ヘッダーおよびフッターにも、清潔感のある救援用（LifeBuoy）アイコンを用いた窓口リンクを追加。
  - note側の規約や日本人ユーザーの心理的ハードルを考慮し、Amazonアソシエイト等の広告を排した直接支援・対話モデルを採用。

## [1.5.6] - 2026-02-23

### バグ修正
- **既存データ再編集時のvideo_urlの保存漏れ（消失バグ）を修正**:
  既存の投稿URLをLoadして上書き保存（Update）する処理において、データベースへの送信ペイロードから `video_url` が抜け落ちていたため、画面上で動画URL（.mp4）が正常に生成されていてもDB側では空のまま更新されない抜け道を修正しました。

## [1.5.5] - 2026-02-22

### 修正・UI改善
- **モバイル表示（スマホ表示）のレイアウト崩れや窮屈さを全面解消**:
  - 投稿画面（ShareInput）にて、URL入力欄とLoadボタンを常に横並びに固定し、プレビュー画像の縦長デッドスペースを排除してコンパクト化しました。
  - ギャラリー一覧（VideoCard）にて、詳細ボタン等を「アイコン＋短縮テキスト」に変更し、画面幅が狭い際に文字が押しつぶされて不格好に強制改行されるレイアウト崩壊を解消しました（flex-wrapによる安全な折り返し設計を導入）。
  - 各種詳細モーダルにて、ヘッダー右上のボタン群とタイトル文字が被り操作しづらかった問題を排除し、フレックスレイアウトで自然に配置される作りに修正しました。
