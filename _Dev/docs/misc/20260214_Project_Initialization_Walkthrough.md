...
- **バックエンド API**:
    - `GET /api/posts`: 共有された生成物を一覧表示。`?q=` による検索をサポート。
    - `POST /api/posts`: 新しい生成物を保存。
    - `GET /api/preview`: **Playwright** を使用して Grok URL から OpenGraph メタデータをスクレイピング。
- **フロントエンド**:
    - **ShareInput**: Grok URLを貼り付け -> プレビュー -> 共有。
    - **VideoCard**: ホバーで再生される動画プレビュー。
