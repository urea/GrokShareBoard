...
async function migrateIds() {
    ...
    for (const post of posts) {
        ...
        const grokUuid = uuidMatch[1];

        const { data: existingTarget } = await supabase.from('posts').select('id').eq('id', grokUuid).single();
        if (existingTarget) {
            console.log(`[Skip] Already migrated: ${grokUuid}`);
            skipCount++;
            continue;
        }
        ...
        // EXCLUDE 'search_tsv' generated column
        const { search_tsv, ...postData } = post;
        const newPost = { ...postData, id: grokUuid };
